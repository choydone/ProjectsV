...
import {LoginLoggerService} from "../../../shared/admin/login-logger.service";
import {R} from "../../../starter/utils/utils";
import {WebCommandService} from "../../../starter/shared/web-command-service";

@Component({
  selector: 'app-login-logger-list',
  templateUrl: './login-logger-list.component.html',
  styleUrls: ['./login-logger-list.component.scss']
})
export class LoginLoggerListComponent  extends BaseComponent {
  ...
  constructor(private fb: FormBuilder, private router: Router, private message: NzMessageService,
              securityService: SecurityService, webCommandService: WebCommandService, private loginLoggerService: LoginLoggerService) {
  ...
  search(reset: boolean = false): void {
    ...
    const observable: Observable<R> = this.loginLoggerService.searchAtPage(query);
