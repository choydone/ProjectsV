...
@Component({
  selector: 'app-copyright-auto-console',
  templateUrl: './copyright-auto-console.component.html',
  styleUrls: ['./copyright-auto-console.component.scss']
})
export class CopyrightAutoConsoleComponent implements OnInit, OnDestroy {
  ...
  startTestAction(): void {
    ...
    this.messages = [];
    this.docxList = [];
    const observable: Observable<R> = this.autoScriptService.testCodeAction(this.currentDataset.id);
        ...
        this.stompClient.connect({}, (frame: any) => {//3
          ...
          this.stompClient.subscribe('/userTest/U_' + user.id + "_" + this.currentDataset.id + "/info", (message: any) => {
            ...
            if (message.body.includes("【Success】")) { // 表示测试成功,字符串不能修改
              this.callbackTestResultEvent.emit(true);
            ...
            if (message.body.includes("【Error】")) { // 表示测失败,字符串不能修改
              this.callbackTestResultEvent.emit(false);
              this.nzModalBtnVisible = false;
            }
