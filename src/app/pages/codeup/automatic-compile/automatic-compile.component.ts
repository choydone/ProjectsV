...
@Component({
  selector: 'app-automatic-compile',
  templateUrl: './automatic-compile.component.html',
  styleUrls: ['./automatic-compile.component.scss']
})
export class AutomaticCompileComponent implements OnInit, OnDestroy {
  ...
  startCompile(): void {
    this.messages = [];
    const observable: Observable<R> = this.autoScriptService.resetAction();
    ...
    observable.subscribe((data) => {
      ...
      let connect = () => {
        this.stompClient.connect({}, (frame: any) => {//3
          ...
          //监听
          this.stompClient.subscribe('/userTest/U_' + user.id + "_100" + "/info", (message: any) => {
